...
export default function ModernContactCategory({
  ...
  // Completion Status
  const isComplete = Boolean(step5.contactPerson && step5.contactPhone && step5.department);
  ...
  return (
    <div className="w-full space-y-4 px-4 pb-20 sm:space-y-6 sm:px-6 lg:px-8">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 p-6 shadow-2xl sm:p-8">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white sm:text-3xl">Kontaktinformationen</h2>
              <p className="mt-1 text-sm text-white/80 sm:text-base">Wie kann die Öffentlichkeit helfen?</p>
            </div>
          </div>
          {/* Progress Bar */}
          <div className="mt-6">
            ...
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/20">
              <div className="h-full rounded-full bg-white transition-all duration-500" style={{ width: `${completionPercentage}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-3">
        <div className="space-y-4 sm:space-y-6 lg:col-span-2">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-sm font-medium">
                  ...
                  <Badge variant="destructive" className="ml-1 h-5 px-1.5 text-xs">Pflicht</Badge>
                </Label>
                {isEditMode ? (
                  <div className="relative">
                    <Input
                      value={step5.contactPerson}
                      onChange={(e) => updateField("step5", "contactPerson", e.target.value)}
                    ...
                    />
                    {step5.contactPerson && <CheckCircle2Icon className="absolute right-3 top-3.5 h-5 w-5 text-green-500" />}
                  </div>
                ) : (
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-sm font-medium">
                  <PhoneCallIcon className="h-4 w-4 text-gray-500" /> Telefonnummer
                  <Badge variant="destructive" className="ml-1 h-5 px-1.5 text-xs">Pflicht</Badge>
                </Label>
                {isEditMode ? (
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => updateField("step5", "contactPhone", e.target.value)}
                      ...
                      type="tel"
                    />
                    {step5.contactPhone && <CheckCircle2Icon className="absolute right-3 top-3.5 h-5 w-5 text-green-500" />}
                  </div>
                ) : (
                  <div className="flex items-center gap-3 rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-sm font-medium">
                  ...
                  <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-xs">Optional</Badge>
                </Label>
                {isEditMode ? (
                  <div className="relative">
                    <Input
                      ...
                      type="email"
                    />
                    {step5.contactEmail && !emailError && <CheckCircle2Icon className="absolute right-3 top-3.5 h-5 w-5 text-green-500" />}
                  ...
                  step5.contactEmail && (
                    <div className="flex items-center gap-3 rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">{step5.contactEmail}</span>
                    </div>
                  )
                )}
              </div>
                ...
                {isEditMode ? (
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => updateField("step5", "department", e.target.value)}
                    ...
                    />
                    {step5.department && <CheckCircle2Icon className="absolute right-3 top-3.5 h-5 w-5 text-green-500" />}
                  </div>
                ) : (
                  <div className="flex items-center gap-3 rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
                    <ShieldIcon className="h-5 w-5 text-gray-500" />
                    <span className="font-medium text-gray-900 dark:text-white">{step5.department || "Nicht angegeben"}</span>
                  </div>
                )}
              </div>
              {/* Available Hours */}
              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-sm font-medium">
                  <CalendarClockIcon className="h-4 w-4 text-gray-500" /> Erreichbarkeit
                  <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-xs">Optional</Badge>
                </Label>
                {isEditMode ? (
                  <Textarea
                    value={step5.availableHours}
                    onChange={(e) => updateField("step5", "availableHours", e.target.value)}
          ...
          {isEditMode && (
            <Card className="border-0 bg-gradient-to-r from-purple-50 to-pink-50 shadow-sm dark:from-purple-950 dark:to-pink-950">
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2 text-lg">
                  <SparklesIcon className="h-5 w-5 text-purple-600" /> Erweiterte Kontaktoptionen
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Button variant="outline" className="h-auto flex-col items-center justify-center gap-2 p-4" type="button">
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* Status Card */}
          <Card className={`border-0 ${isComplete ? "bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950 dark:to-emerald-950" : "bg-white dark:bg-gray-800"} shadow-sm`}>
            <CardContent className="p-6">
              <div className="flex flex-col items-center text-center">
                <div className={`flex h-16 w-16 items-center justify-center rounded-full ${isComplete ? "bg-green-100 dark:bg-green-900" : "bg-yellow-100 dark:bg-yellow-900"}`}>
                  {isComplete ? <CheckCircle2Icon className="h-8 w-8 text-green-600 dark:text-green-400" /> : <AlertCircleIcon className="h-8 w-8 text-yellow-600 dark:text-yellow-400" />}
                </div>
                  ...
                  <div className="mt-4 space-y-2 text-left">
                    {!step5.contactPerson && (
                      <div className="flex items-center gap-2 text-xs text-red-600">
                        <div className="h-1.5 w-1.5 rounded-full bg-red-600" /> Kontaktperson fehlt
                      </div>
                    )}
                    {!step5.contactPhone && (
                      <div className="flex items-center gap-2 text-xs text-red-600">
                        <div className="h-1.5 w-1.5 rounded-full bg-red-600" /> Telefonnummer fehlt
                      </div>
                    )}
                    {!step5.department && (
                      <div className="flex items-center gap-2 text-xs text-red-600">
                        <div className="h-1.5 w-1.5 rounded-full bg-red-600" /> Abteilung fehlt
                      </div>
                    )}
                  </div>
                )}
          ...
          {!isEditMode && isComplete && (
            <Card className="border-0 bg-white shadow-sm dark:bg-gray-800">
              ...
              <CardContent className="space-y-3">
                <a href={`tel:${step5.contactPhone}`} className="flex items-center gap-3 rounded-lg bg-blue-50 p-3 transition-colors hover:bg-blue-100 dark:bg-blue-950 dark:hover:bg-blue-900">
                  <PhoneIcon className="h-5 w-5 text-blue-600" />
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">Anrufen</p>
                    <p className="text-xs text-gray-600 dark:text-gray-400">{step5.contactPhone}</p>
                  </div>
                </a>
                {step5.contactEmail && (
                  <a href={`mailto:${step5.contactEmail}`} className="flex items-center gap-3 rounded-lg bg-purple-50 p-3 transition-colors hover:bg-purple-100 dark:bg-purple-950 dark:hover:bg-purple-900">
                    <MailIcon className="h-5 w-5 text-purple-600" />
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white">E‑Mail senden</p>
                      <p className="text-xs text-gray-600 dark:text-gray-400">{step5.contactEmail}</p>
                    </div>
                  </a>
                )}
              </CardContent>
          ...
          <Card className="border-0 bg-gradient-to-br from-blue-50 to-indigo-50 shadow-sm dark:from-blue-950 dark:to-indigo-950">
            <CardContent className="p-6">
              ...
              <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <li className="flex items-start gap-2"><div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-blue-600" /><span>Stellen Sie sicher, dass die Kontaktdaten aktuell sind</span></li>
                <li className="flex items-start gap-2"><div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-blue-600" /><span>Geben Sie alternative Kontaktmöglichkeiten an</span></li>
                <li className="flex items-start gap-2"><div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-blue-600" /><span>Prüfen Sie die Erreichbarkeitszeiten</span></li>
              </ul>
            </CardContent>
          </Card>
        </div>
      ...
      {isEditMode && (
        <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800 sm:relative sm:border-0 sm:bg-transparent sm:p-0 sm:dark:bg-transparent">
          <div className="flex gap-3">
            <Button onClick={onPrevious} variant="outline" className="flex-1 sm:flex-none">
              <ChevronLeftIcon className="mr-2 h-4 w-4" /> Zurück
            </Button>
            <Button onClick={onSave} disabled={!isComplete} className={`flex-1 sm:flex-none ${isComplete ? "bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700" : ""}`}>
